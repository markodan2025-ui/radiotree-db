...
def fetch_codec(server, codec, limit=1000):
    """Pometi SVE stanice s tim codecom, bez obzira na oznake.

    Ovo je razlog zasto smo prvi put nasli samo 23 lossless stanice:
    trazili smo po oznakama 'flac'/'lossless', a vecina FLAC stanica
    tu oznaku uopce nema. Radio Browser zna codec za svaku stanicu -
    pitaj ga izravno.
    """
    url = f"{server}/json/stations/search"
    params = {"codec": codec, "limit": limit, "hidebroken": "true",
...
def fetch_bitrate(server, minbr, limit=1000):
    """Sve stanice iznad zadanog bitratea, bez obzira na oznake."""
    url = f"{server}/json/stations/search"
    params = {"bitrateMin": minbr, "limit": limit, "hidebroken": "true",
              "order": "clickcount", "reverse": "true"}
    r = requests.get(url, params=params, headers={"User-Agent": UA}, timeout=60)
    r.raise_for_status()
    return r.json()
...
def map_tags_to_genres(st, genres):
    """Pretvori slobodne RB oznake u nase zanrove."""
    raw = (st.get("tags") or "").lower()
    out = []
    for g in genres:
        for t in g["tags"]:
            if t.lower() in raw:
...
def main():
    ...
    # --- SWEEP: sve po codecu i bitrateu, neovisno o oznakama ----------------
    sweeps = []
    if os.environ.get("CODEC_SWEEP", "1") == "1":
        for codec in ("FLAC", "ALAC", "WAV"):
            try:
                sweeps.append((codec.lower(), fetch_codec(server, codec)))
            except Exception as e:
                print(f"  ! sweep {codec}: {e}")
        try:
            sweeps.append(("hi-bitrate", fetch_bitrate(server, 256)))
        except Exception as e:
            print(f"  ! sweep bitrate: {e}")
    ...
    for label, rows in sweeps:
        ...
        for st in rows:
            ...
            sid = station_id(st)
            key = norm_key(st.get("url_resolved") or st.get("url"))
            gl = map_tags_to_genres(st, genres)
            lossless = (st.get("codec") or "").upper() in ("FLAC", "ALAC", "WAV", "PCM")
            if lossless and "audiophile" not in gl:
                gl.append("audiophile")
            if not gl:
                gl = ["audiophile"] if lossless else ["other"]
            if key in seen_keys:
                prev = by_id.get(seen_keys[key])
                if prev:
                    for g in gl:
                        if g not in prev["genres"]:
                            prev["genres"].append(g)
                continue
            seen_keys[key] = sid
            if sid in by_id:
                for g in gl:
                    if g not in by_id[sid]["genres"]:
                        by_id[sid]["genres"].append(g)
                continue
            by_id[sid] = {
                "source": "radio-browser",
                "id": sid,
